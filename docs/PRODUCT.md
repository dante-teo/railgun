...
**Built-in web access:**
Railgun includes `web_search` and `web_fetch` as zero-configuration, read-only
...
`railgun cron install` (macOS/Linux only, throwing an unsupported platform error on Windows) registers `railgun cron` as a persistent OS-managed service — launchd on macOS (`~/Library/LaunchAgents/sh.railgun.cron.plist`, `RunAtLoad: true`, `KeepAlive: true`) or a systemd user service (`~/.config/systemd/user/railgun-cron.service`, `Restart=always`). It also registers a hidden nightly `railgun dream` task at local midnight (a launchd calendar interval on macOS or the `railgun-dream.timer` systemd timer on Linux). This task is intentionally not a `CronJob`: it is absent from `jobs.json` and `/cron` listings, and it uses the normal dream command's minimum-five-memory guard and memory/SOUL.md writes. The daemon starts at login and restarts on crash, running cron jobs continuously without any interactive railgun session open. `railgun cron uninstall` removes both services and `railgun cron status` reports the public scheduler service. The scheduler manages its own log files at `~/.railgun/cron/logs/cron-YYYY-MM-DD.log` (rotated daily at UTC midnight, 7-day retention); a `cron-latest.log` symlink in that directory always points to today's UTC date file. Each log line is prefixed with an ISO-8601 timestamp; per-job output includes turn count, tool calls with truncated args, and a completion summary with duration. The daemon redirects its own stdout/stderr to `/dev/null` since all relevant output is written by the scheduler logger. The foreground `railgun cron` command remains for users who want an explicit manual daemon or a dedicated terminal session. Crucially, the persistent OS-managed service (`install`, `uninstall`, `status` subcommands) only works after global installation (`pnpm add --global @dantea/railgun`) and is not supported when running from a repo checkout (e.g. `pnpm start cron install` will not work, as the daemon requires a globally resolved binary path).
