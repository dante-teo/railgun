...
- [x] `SWFT-022` — Verify arm64 packaged backend startup, SQLite loading, authentication startup, crash, restart, and shutdown. `[8h]`

Milestone exit: arm64 runs the bundled backend through RPC v1,
...
- [x] `SWFT-024` — Build the shell using native split navigation, toolbar actions, list selection, and a toolbar-anchored Activity popover. `[8h]`
...
- [x] `SWFT-030` — Build tool activity and populate the Activity toolbar popover using native grouping, lists, button expanders, and shared presentation variants. `[8h]`
...
- [x] `SWFT-034` — Implement model selection and persistence with a native menu while consuming configured MoA and advisor behavior without main-page toggles. `[8h]`
- [x] `SWFT-086` — Implement optional context-ring presentation and model-menu manual compaction, including availability, progress, completion, failure, and restored-session behavior. `[8h]`
...
- [x] `SWFT-087` — Implement summarization and fork workflows, including progress, failure, cancellation, and authoritative post-operation reloads. `[8h]`
- [x] `SWFT-047` — Build the home-rooted file tree using native outline/list behavior and filesystem protections. `[8h]`
...
after sources change. Generated projects place the clean Swift package checkout
at `../SourcePackages` relative to `PROJECT_DIR`; the `RailgunUI` Markdown
header workaround intentionally uses that layout.
