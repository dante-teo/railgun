...
| Desktop WS gateway (`apps/desktop/gateway/`) | Node.js WebSocket server (`ws` package, `localhost:0` random port) that bridges one `AgentSession` to a browser renderer. `protocol.ts` defines `GatewayCommand` (client → server) and `GatewayEvent` (server → client) JSON frame types. `sessionManager.ts` wraps `createAgentSession` with the same callback pattern as `rpcMode.ts`: `confirmShellCommand` and `clarifyCallback` use `Promise.withResolvers()` to park the agent, emit an `approval_request` or `clarify_request` frame, and resolve when the client sends `approve` or `clarify_response`. `wsServer.ts` accepts one client at a time and dispatches the 12 command variants via an exhaustive switch. A `state_update` frame is sent after each run slot clears. See `docs/adr/0028-desktop-app-ws-gateway.md`. | Solo project |
| Desktop renderer (`apps/desktop/renderer/`) | React 19 DOM app (Vite + `@vitejs/plugin-react`, `react-markdown` + `remark-gfm` for assistant messages). Mirrors the visual and interaction contract of `src/repl/App.tsx` in a browser context. **CSS layer**: `styles/tokens.css` maps all 16 `Palette` fields plus `--color-page-bg` to CSS custom properties (dark default via `:root`, light via `[data-theme="light"]`), with `--transition-theme` providing 120 ms ease transitions on color switches; `styles/layout.css` covers the full component class surface (app shell, transcript, composer, status bar, message bubbles, tool-call rows, todo panel, overlays, slash suggestions); `styles/markdown.css` scopes assistant-message Markdown. **Theme**: `lib/theme.ts` exports `applyTheme`/`getInitialTheme`/`subscribeThemeChanges`; theme is applied by writing/removing `data-theme="light"` on `document.documentElement`. **Production shell** (T7): `lib/gatewayClient.ts` — typed WebSocket wrapper (`createGatewayClient(url)`) with auto-reconnect (1 s–8 s exponential backoff), request/response correlation by `id`, 10 s timeout, and a subscriber `Set` for `GatewayEvent` fan-out. `lib/useAgentEvents.ts` — the single hook owning the client, mapping every `GatewayEvent` to `useReducer` state (`lines`, `streaming`, `busy`, `toolLabels`, `todos`, `overlay`, `composerMode`, `connected`, `pendingCommand`, `pendingClarify`, `availableModels`), and exposing typed action callbacks. `components/App.tsx` — production entry point (`import.meta.env.DEV` false); wires all components to live state, computes ephemeral pending tool-label lines via `useMemo` (streaming text excluded — `Transcript` renders it via its own `streaming` prop to avoid duplication), and handles slash commands locally. `DevShell` remains the dev-mode entry for UI iteration without a running gateway. **`@railgun/core` import boundary**: the renderer must not import values from any `src/repl/` module that transitively imports `ink` — doing so triggers a top-level-await Vite build error in the browser target. Safe value imports from core: `src/repl/toolLineStyle.js` (`shouldAppendToolTranscriptLine`, `shouldShowToolLine`), `src/repl/streamingTranscript.js`, `src/tools/toolLabel.js`, `src/advisor/advisoryMessage.js`, `src/tools/todo.js`, `src/commands.js`. Type-only imports from ink-bearing modules such as `src/repl/App.js` (`DisplayLine`) are safe and do not pull `ink` into the bundle. **Test harness**: `lib/test/fakeWebSocket.ts` exports `installFakeWebSocket()` — stubs `globalThis.WebSocket` with a synchronous `FakeWebSocket` class and returns `{ simulateOpen, simulateMessage, simulateClose, rawSent }` primitives used by both `gatewayClient.test.ts` and `useAgentEvents.test.ts`. See `docs/adr/0028-desktop-app-ws-gateway.md`. | Solo project |
...
- The desktop app (`apps/desktop/`) is a pnpm workspace package. During development: `pnpm dev` (inside `apps/desktop/`) starts `electrobun dev --watch`, which builds the Bun main process and the React renderer view, then launches the native window; the gateway runs as a separate Node.js process. For production (Sprint 3+): `electrobun build --env=stable` produces a self-contained app bundle (~14 MB with system webview). The runtime is Electrobun (Bun-based; see `docs/adr/0031-electrobun-desktop-runtime.md`); the renderer runs in the platform system webview (WKWebView/WebView2/WebKitGTK).
...
| Note store (`src/persistence/noteStore.ts`) | FTS5 and vector-search prepared-statement wrapper around the shared SQLite connection (Phases 26–27): `importFolder(folderPath, chunkWords?)` reads `.md`/`.txt` files from the top-level directory, splits content into word chunks (default 500 words), and inserts each chunk inside a single transaction; `search(query, limit?)` sanitizes the query (strips FTS5 syntax characters), runs a `MATCH` query against `notes_fts` with `snippet()` extraction, and returns ranked results with `sourcePath` and `snippet` fields; Phase 27 adds `storeVector(noteId, embedding)`, `searchSemantic(queryVector, limit?)`, `importFolderWithEmbeddings(folderPath, embedFn, chunkWords?)`, and `backfillEmbeddings(embedFn)` against the `notes_vec` sqlite-vec virtual table (`embedding FLOAT[384]`), plus an `EmbedFn` type. The `notes_fts` virtual table is kept in sync by three database triggers (insert/delete/update) defined in the schema migration | Solo project |
9: Architecture decisions are tracked in `docs/adr/`. Use short, dated records for decisions that meaningfully affect structure, dependencies, operations, or long-term maintenance. See `docs/adr/0013-command-risk-gate-and-smart-approval.md` for the Phase 21 risk-gate design, `docs/adr/0014-mcp-client-support.md` for the Phase 24 MCP client, `docs/adr/0014-persistent-memory.md` for the Phase 25 memory design, `docs/adr/0014-advisor-passive-second-model.md` for the Phase 34 advisor design, `docs/adr/0015-skills-system.md` for the Phase 28 skills system design, `docs/adr/0016-ansi-first-ui-design-system.md` for the Phase 35 shared ANSI design system, `docs/adr/0016-delegate-task-subagent-spawning.md` for the Phase 31 delegation design, `docs/adr/0026-notes-fts5-search.md` for the Phase 26 FTS5 notes search design, and `docs/adr/0027-semantic-note-search.md` for the Phase 27 semantic note search design.
See `docs/adr/0028-desktop-app-ws-gateway.md` for the desktop WebSocket gateway and renderer design (Sprint 0–1). See `docs/adr/0031-electrobun-desktop-runtime.md` for the Electrobun runtime decision (T9).
