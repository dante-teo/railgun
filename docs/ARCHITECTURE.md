...
| Desktop renderer (`apps/desktop/renderer/`) | React 19 DOM app (Vite + `@vitejs/plugin-react`, `react-markdown` + `remark-gfm` for assistant messages). Mirrors the visual and interaction contract of `src/repl/App.tsx` in a browser context. **CSS layer**: `styles/tokens.css` maps all 16 `Palette` fields plus `--color-page-bg` to CSS custom properties (dark default via `:root`, light via `[data-theme="light"]`), with `--transition-theme` providing 120 ms ease transitions on color switches; `styles/layout.css` covers the full component class surface (app shell, transcript, composer, status bar, message bubbles, tool-call rows, todo panel, overlays, slash suggestions); `styles/markdown.css` scopes assistant-message Markdown. **Theme**: `lib/theme.ts` exports `getInitialTheme`/`applyTheme`/`subscribeThemeChanges` — dark is the CSS default (no attribute), light sets `data-theme="light"` on `<html>`. **Hooks**: `useComposer` (draft + Tab-completion state machine exposing `draft`, `completionIndex`, `completionMatches`, `liveMatches`, `setDraft`, `handleTab`, `handleEscape`, `handleCtrlU`, `handleSubmit`); `useOverlayKeyNav` (shared Arrow/Enter/Escape handler for list overlays; Escape is unconditional, arrow/Enter gated on `length > 0`; `wrap` flag selects clamp vs. wrap-around). **Components**: `Transcript` (auto-scroll + unseen pill), `MessageBubble` (5 line kinds; partial-with-text → Markdown + `StreamingCursor`; partial-without-text → "Thinking" indicator with `.thinking-text` CSS class; streaming content lives in `line.text` of the ephemeral partial `DisplayLine` — not a separate prop), `Composer` (auto-grow textarea, all TUI key bindings), `SlashSuggestions`, `TodoPanel` (summary header via `summarizeTodos()` showing `completed/total`, items in a `div[role="list"]` wrapper to satisfy ARIA — header sits outside as a plain sibling), `StatusBar` (git branch + dirty dot + `cwd` string prop in left section with `·` separator, model name, MoA preset, unsaved indicator in right section — `cwd` is presentational; gateway sourcing deferred to T7), and six overlays — `ModelPicker`/`ActionPicker` (clamp), `TrustPicker`/`SessionChooser` (wrap; `SessionChooser` also binds Ctrl+C), `ClarifyPrompt` (choice list or free-text with bespoke handler; `:focus-visible` ring on free-text input), `ShellApproval` (y/n/Escape). `DevShell` (dev-only) simulates thinking → streaming → settled on each submit; re-entrant submits while busy are dropped. Covered by 41 Vitest/jsdom component tests (`@testing-library/react`, per-file `// @vitest-environment jsdom`). Gateway wiring is Sprint 2. See `docs/adr/0028-desktop-app-ws-gateway.md`. | Solo project |
