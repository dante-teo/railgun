...
| Session store (`src/persistence/sessionStore.ts`) | Functional factory around synchronous SQLite: schema v2 (`memories` table added in Phase 25), versioned migration, strict message/todo codecs, fail-closed transcript validation, newest-first summaries, atomic idempotent full-snapshot checkpoints, and a `readonly db` handle exposed for `MemoryStore` to share the connection | Solo project |
...
| Streaming transcript (`src/repl/streamingTranscript.ts`) | Pure segment state that accumulates deltas, flushes narration before tools and queued-user injection, and returns only the uncommitted final/aborted assistant suffix | Solo project |
  | Command system (`src/commands.ts`) | Pure `/exit`, `/help`, `/clear`, `/model`, `/compact`, `/rollback`, `/trust`, and `/moa` matching, parsing, and tab/escape completion state; no I/O or React | Solo project |
...
todo snapshots, and user memories (since Phase 25 — schema v2, `memories` table).
It uses WAL, foreign keys, a busy timeout, and schema versioning with migration;
malformed saved state aborts loading instead of being skipped. All three session
modes open this database as of Phase 25 to load memories into the system prompt.
See ADR 0006.
