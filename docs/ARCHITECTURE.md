...
| --- | --- | --- |
| CLI entry (`src/cli.ts`) | Pure argv parsing plus injectable dispatch for `config`, `login`, `logout`, `cron`, fresh REPL, exact/interactive resume, session listing, stateless one-shot, and headless RPC modes; config/auth/cron commands return before SQLite/session/TUI boundaries | Solo project — no formal ownership split |
...
| Built-in tools (`src/tools/{readFile,writeFile,listDirectory,runShell,todo,clarify,memory,delegate}.ts`) | Eight self-registering tools: file I/O, caller-owned todo planning, `run_shell_command`, `clarify`, `memory_write`, and `memory_search`; shell execution is routed through `checkCommandApproval` first — hardline-blocked commands return immediately as errors, safe commands execute directly, and dangerous commands go through configurable approval (manual y/n prompt, LLM smart review, or off); approved dangerous patterns are added to the per-session `sessionApprovals` set so the same class does not re-prompt within one conversation; the shell child is detached into a POSIX process group, sent `SIGTERM` on abort, then `SIGKILL` after a two-second grace period; `clarify` routes a question (with optional up-to-4 choices) to the injected `ClarifyCallback` and returns `{ question, answer }` JSON; memory tools read/write the `MemoryStore` injected via `ToolContext`; the self-registering `advise` tool (toolset `"advisory"`) is available only when `advisoryContext` is present on `ToolContext` — three emission guards (content-free phrase suppression, per-runtime dedupe, one-note-per-update rate limit) keep it quiet; `"concern"`/`"blocker"` notes are routed to the steering queue, `"nit"` notes are appended directly to `primaryMessages` | Solo project |
...
| Memory store (`src/persistence/memoryStore.ts`) | Prepared-statement wrapper around the shared SQLite connection: `save(content, category)` inserts a UUID-keyed memory row; `search(query, limit?)` returns case-insensitive LIKE matches newest-first (with `rowid DESC` as tiebreaker for same-millisecond inserts); `recent(limit?)` returns the most recent memories; `formatMemoriesForPrompt` formats a list for system-prompt injection, returning `null` for empty lists. Categories: `"preference"`, `"fact"`, `"project"`. Phase 26 will replace LIKE search with FTS5 | Solo project |
